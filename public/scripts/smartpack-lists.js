var smartpackAdmin = (function() {
    ...
    _this.init = function(listData, listTypes) {
    ...
    _this.updateListIndex = function(newpos) {
        var table = $(".smartpack-lists");
        console.log($(this));
    }
    ...
    _this.addBox = function(data) {
        if (data) {
            ...
            var html = ' \
                <tr data-list-id = "' + id + '"id="list-id-' + id + '" class="' + dirtyClass + '"> \
                    <td> \
                      <div class="rule-status" style="" data-toggle="buttons">\
                        <button class="btn btn-sm btn-success btn-status ' + checked + '" onclick="changeState(this)">\
                            ON\
                        </button>\
                        <button class="btn btn-sm btn-default btn-status ' + unchecked + '" onclick="changeState(this)">\
                            OFF\
                        </button>\
                      </div>\
                    </td> \
                ...
                <tr id="list-contents-' + id + '" class="' + dirtyClass + '" style="display:none">\
                    ...
                    <div class="row btns-row" style="margin-bottom:10px;"> \
                        ...
                        <div class="col-md-6"> \
                            <div class="pull-right"> \
                                <button onclick="smartpackAdmin.runUploader(this)" class="btn btn-default">Import</button> \
                                <button onclick="smartpackAdmin.exportList(this)" class="btn btn-default">Export</button> \
                                <div class="btn-group">\
                                    <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" onclick="smartpackAdmin.resetOptions(this)">More Actions&nbsp;&nbsp;<i class="fa fa-caret-down"></i></button>\
                                        ...
                                        <div style="width:320px">\
                                            <div class="form-horizontal">\
                                              <div class="form-group row">\
                                                ...
                                                <div class="col-sm-6">\
                                                    <button onclick="smartpackAdmin.clearList(this)" class="btn btn-danger btn-block">Clear List</button>\
                                                </div>\
                                              </div>\
                                              <div class="form-group row">\
                                                <label for="list-type" class="col-sm-6 form-control-label" style="font-weight:normal">Permanently Delete List</label>\
                                                <div class="col-sm-6">\
                                                    <button onclick="smartpackAdmin.deleteBox(this)" class="btn btn-danger btn-block">Delete List</button>\
                                                </div>\
                                              </div>\
                                            </div>\
                                        </div>\
                                ...
                                <button onclick="smartpackAdmin.publishBox(this)" class="btn btn-primary publish">Publish</button> \
                            </div> \
                        </div> \
                    </div> \
                    ...
                    <div class="simplegrid-full list-footer"> \
                        <button onclick="smartpackAdmin.addListItem(this)" class="btn btn-primary">Add New Item</button> \
                    </div>\
                    </div> \
                    </td> \
                </tr>\
            ...
            $('#smartpack-lists tr:last').after(html);
            ...
            $('#list-id-' + id).find('.list-name').editable({
                ...
                params: function(params) {
                    ...
                    var list_id = $(parent).data('list-id');
                ...
                success: function(response, newValue) {
                    ...
                    if (response.status != "error") {
                        ...
                        $('#list-contents-new').find('.list-description').editable('option', 'url', '/smartpack/save');
                        $('#list-contents-new').find('.list-description').editable('option', 'pk', 1);

                        $('#list-id-new').attr("id", 'list-id-' + response['list-id']).attr('data-list-id', response['list-id']);
                        $('#list-contents-new').attr("id", 'list-contents-' + response['list-id']);
            ...
            $('#list-id-' + id).find('.list-content-toggle').bind('click', function() {
                _this.expandColapse(this);
            });
            ...
            if (id !== 'new') {
                ...
                optsDescription['url'] = '/smartpack/save';
            }
    ...
    _this.saveBox = function(obj) {
        ...
        var _save = function() {
            $.ajax({
                url: "/smartpack/save",
                type: "POST",
                data: "list_id=" + list_id + "&list_status=" + list_status + "&list_name=" + list_name,
                ...
                success: function(data) {
                    if (data.status != "error") {
                        ...
                        if (data.messages.length === 0) {
                            $(".modal-body #missing-sku-textarea").val("All items exists in catalog!");
                        } else {
                            $(".modal-body #missing-sku-textarea").val(data.messages);
                        }
                        $('#missing-sku-in-catalog-form').modal('show');
                    } else {
                        notify("error", data.message);
                    }
                },
    ...
    _this.searchAll = function() {
        ...
        $.ajax({
            url: "/smartpack/listitems/search-all",
            data: $.param(params, true),
            dataType: 'json',
            success: function(data) {
                if (data.status != "error") {
    ...
    _this.collapseAll = function() {
        $('#smartpack-lists tr').filter(function() {
    ...
    _this.initializeIframe = function(list_id, searchStr) {
        ...
        var link = "/smartpack/listitems?" + $.param(params);
    ...
    _this.exportList = function(obj) {
        ...
        var link = "/smartpack/listitems/export/?list-id=" + list_id;
    ...
    _this.addListItem = function(obj) {
        ...
        //var list_type_display = list_type = 'SKU';
        ...
        var modalTitle = 'Add item to list';
    ...
    _this.deleteListItem = function(item_id, list_id, callback) {
        ...
        $.ajax({
            url: "/smartpack/listitems/delete",
            dataType: "json",
            type: "POST",
    ...
    _this.changeState = function(obj) {
        ...
        if (list_id == "new") {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "/smartpack/change-state",
                    dataType: "json",
                    type: "POST",
                    data: "list-id=" + list_id + "&list-status=" + list_status + "&list-name=" + list_name,
                    success: function(data) {
                        if (data.status != "error") {
                            notify("success", data.message);
