...
;(function(window, $, undefined) {
	...
	$.url = function( opts )
	{
        ...
        return {
            ...
            param : function( param )
            {
            	...
            	if(param !== undefined){
            		result = this.data.param.query[param];
            		if(result == undefined){
            			result = this.data.friendly[param];
            			if(result == undefined){
            				result = null;	
            			}
            		}
	...
	$.url.replaceQueryString = function replaceQueryString(url, param, value) {  
			var preURL = "";
			var postURL = "";
			var newURL = "";
			...
			else if(url.indexOf(param + "/") > -1){//Not found, searching for friendly URL format
				start = url.indexOf(param + "/");
				end = url.indexOf("/", start);
				preURL = url.substring(0, end) + "/" + value;
				var postUrlEnd = url.indexOf("/", end+1);
				//Attemp to find a GET paramer (non-friendly)
				if(postUrlEnd == -1)
					postUrlEnd = url.indexOf("?", end+1);
				if(postUrlEnd == -1)
					postUrlEnd = url.indexOf("&", end+1);
